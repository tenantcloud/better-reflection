...
namespace TenantCloud\BetterReflection\Relocated\Nette\Loaders;
...
class RobotLoader
{
    ...
    public function __construct()
    {
        if (!\extension_loaded('tokenizer')) {
            throw new \TenantCloud\BetterReflection\Relocated\Nette\NotSupportedException('PHP extension Tokenizer is not loaded.');
        }
    }
            ...
            foreach ($iterator as $fileInfo) {
                ...
                foreach ($foundClasses as $class) {
                    if (isset($this->classes[$class])) {
                        throw new \TenantCloud\BetterReflection\Relocated\Nette\InvalidStateException("Ambiguous class {$class} resolution; defined in {$this->classes[$class][0]} and in {$file}.");
                    }
     ...
     */
    private function createFileIterator(string $dir) : \TenantCloud\BetterReflection\Relocated\Nette\Utils\Finder
    {
        if (!\is_dir($dir)) {
            throw new \TenantCloud\BetterReflection\Relocated\Nette\IOException("File or directory '{$dir}' not found.");
        }
        ...
        $iterator = \TenantCloud\BetterReflection\Relocated\Nette\Utils\Finder::findFiles($acceptFiles)->filter(function (\SplFileInfo $file) use(&$disallow) {
    ...
    private function updateFile(string $file) : void
    {
        ...
        foreach ($foundClasses as $class) {
            ...
            if (isset($prevFile)) {
                throw new \TenantCloud\BetterReflection\Relocated\Nette\InvalidStateException("Ambiguous class {$class} resolution; defined in {$prevFile} and in {$file}.");
            }
