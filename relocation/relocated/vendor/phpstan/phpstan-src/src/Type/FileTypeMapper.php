...
use TenantCloud\BetterReflection\Relocated\PHPStan\PhpDoc\NameScopedPhpDocString;
...
class FileTypeMapper
{
    ...
    private \TenantCloud\BetterReflection\Relocated\PHPStan\Parser\Parser $phpParser;
    private \TenantCloud\BetterReflection\Relocated\PHPStan\PhpDoc\PhpDocStringResolver $phpDocStringResolver;
    private \TenantCloud\BetterReflection\Relocated\PHPStan\PhpDoc\PhpDocNodeResolver $phpDocNodeResolver;
    ...
    public function getResolvedPhpDoc(string $fileName, ?string $className, ?string $traitName, ?string $functionName, string $docComment) : \TenantCloud\BetterReflection\Relocated\PHPStan\PhpDoc\ResolvedPhpDocBlock
    {
        ...
        if (!isset($this->inProcess[$fileName][$phpDocKey])) {
            ...
            return \TenantCloud\BetterReflection\Relocated\PHPStan\PhpDoc\ResolvedPhpDocBlock::createEmpty();
        }
    ...
    private function createResolvedPhpDocBlock(string $phpDocKey, \TenantCloud\BetterReflection\Relocated\PHPStan\PhpDoc\NameScopedPhpDocString $nameScopedPhpDocString, string $fileName) : \TenantCloud\BetterReflection\Relocated\PHPStan\PhpDoc\ResolvedPhpDocBlock
    {
        ...
        $this->resolvedPhpDocBlockCache[$phpDocKey] = \TenantCloud\BetterReflection\Relocated\PHPStan\PhpDoc\ResolvedPhpDocBlock::create($phpDocNode, $phpDocString, $fileName, $nameScope, $templateTypeMap, $templateTags, $this->phpDocNodeResolver);
        return $this->resolvedPhpDocBlockCache[$phpDocKey];
    }
    private function resolvePhpDocStringToDocNode(string $phpDocString) : \TenantCloud\BetterReflection\Relocated\PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocNode
    {
        $phpDocParserVersion = 'Version unknown';
        try {
            $phpDocParserVersion = \TenantCloud\BetterReflection\Relocated\Jean85\PrettyVersions::getVersion('phpstan/phpdoc-parser')->getPrettyVersion();
        ...
        if ($phpDocNodeSerializedString !== null) {
            ...
            if ($unserializeResult === \false) {
                ...
                if ($error !== null) {
                    throw new \TenantCloud\BetterReflection\Relocated\PHPStan\ShouldNotHappenException(\sprintf('unserialize() error: %s', $error['message']));
                }
                throw new \TenantCloud\BetterReflection\Relocated\PHPStan\ShouldNotHappenException('Unknown unserialize() error');
            }
    ...
    private function shouldPhpDocNodeBeCachedToDisk(\TenantCloud\BetterReflection\Relocated\PHPStan\PhpDocParser\Ast\PhpDoc\PhpDocNode $phpDocNode) : bool
    {
    ...
    private function createFilePhpDocMap(string $fileName, ?string $lookForTrait, ?string $traitUseClass, array $traitMethodAliases = []) : array
    {
        ...
        $functionName = null;
        $uses = [];
        $this->processNodes($this->phpParser->parseFile($fileName), function (\TenantCloud\BetterReflection\Relocated\PhpParser\Node $node) use($fileName, $lookForTrait, $traitMethodAliases, &$phpDocMap, &$classStack, &$namespace, &$functionName, &$uses, &$typeMapStack) : ?int {
            ...
            if ($node instanceof \TenantCloud\BetterReflection\Relocated\PhpParser\Node\Stmt\ClassLike) {
                if ($lookForTrait !== null) {
                    if (!$node instanceof \TenantCloud\BetterReflection\Relocated\PhpParser\Node\Stmt\Trait_) {
                ...
                $resolvableTemplateTypes = \true;
            } elseif ($node instanceof \TenantCloud\BetterReflection\Relocated\PhpParser\Node\Param && $node->flags !== 0) {
                $resolvableTemplateTypes = \true;
                ...
                $resolvableTemplateTypes = \true;
            } elseif (!$node instanceof \TenantCloud\BetterReflection\Relocated\PhpParser\Node\Stmt && !$node instanceof \TenantCloud\BetterReflection\Relocated\PhpParser\Node\Expr\Assign && !$node instanceof \TenantCloud\BetterReflection\Relocated\PhpParser\Node\Expr\AssignRef) {
                return null;
            }
            foreach (\array_reverse($node->getComments()) as $comment) {
                ...
                $phpDocMap[$phpDocKey] = static function () use($phpDocString, $namespace, $uses, $className, $functionName, $typeMapCb, $resolvableTemplateTypes) : NameScopedPhpDocString {
                    ...
                    return new \TenantCloud\BetterReflection\Relocated\PHPStan\PhpDoc\NameScopedPhpDocString($phpDocString, $nameScope);
                };
                if (!$node instanceof \TenantCloud\BetterReflection\Relocated\PhpParser\Node\Stmt\ClassLike && !$node instanceof \TenantCloud\BetterReflection\Relocated\PhpParser\Node\FunctionLike) {
                    continue;
                }
            ...
            if ($node instanceof \TenantCloud\BetterReflection\Relocated\PhpParser\Node\Stmt\Namespace_) {
            ...
            } elseif ($node instanceof \TenantCloud\BetterReflection\Relocated\PhpParser\Node\Stmt\Use_ && $node->type === \TenantCloud\BetterReflection\Relocated\PhpParser\Node\Stmt\Use_::TYPE_NORMAL) {
                ...
                foreach ($node->uses as $use) {
                    if ($node->type !== \TenantCloud\BetterReflection\Relocated\PhpParser\Node\Stmt\Use_::TYPE_NORMAL && $use->type !== \TenantCloud\BetterReflection\Relocated\PhpParser\Node\Stmt\Use_::TYPE_NORMAL) {
                        continue;
                    }
            ...
            return null;
        }, static function (\TenantCloud\BetterReflection\Relocated\PhpParser\Node $node, $callbackResult) use($lookForTrait, &$namespace, &$functionName, &$classStack, &$uses, &$typeMapStack) : void {
            if ($node instanceof \TenantCloud\BetterReflection\Relocated\PhpParser\Node\Stmt\ClassLike && $lookForTrait === null) {
                if (\count($classStack) === 0) {
                    throw new \TenantCloud\BetterReflection\Relocated\PHPStan\ShouldNotHappenException();
                }
                \array_pop($classStack);
            } elseif ($node instanceof \TenantCloud\BetterReflection\Relocated\PhpParser\Node\Stmt\Namespace_) {
                $namespace = null;
                $uses = [];
            } elseif ($node instanceof \TenantCloud\BetterReflection\Relocated\PhpParser\Node\Stmt\ClassMethod || $node instanceof \TenantCloud\BetterReflection\Relocated\PhpParser\Node\Stmt\Function_) {
                $functionName = null;
            }
    ...
    private function getDependentFiles(string $fileName) : array
    {
        ...
        $this->processNodes($this->phpParser->parseFile($fileName), function (\TenantCloud\BetterReflection\Relocated\PhpParser\Node $node) use(&$dependentFiles) {
            ...
            if (!$node instanceof \TenantCloud\BetterReflection\Relocated\PhpParser\Node\Stmt\Class_ && !$node instanceof \TenantCloud\BetterReflection\Relocated\PhpParser\Node\Stmt\Trait_) {
                return null;
            }
            foreach ($node->stmts as $stmt) {
                if (!$stmt instanceof \TenantCloud\BetterReflection\Relocated\PhpParser\Node\Stmt\TraitUse) {
                    continue;
                }
