...
class AnalyserIntegrationTest extends \TenantCloud\BetterReflection\Relocated\PHPStan\Testing\TestCase
{
    ...
    public function testExtendingKnownClassWithCheck() : void
    {
        ...
        $this->assertTrue($broker->hasClass(\TenantCloud\BetterReflection\Relocated\ExtendingKnownClassWithCheck\Foo::class));
    }
    ...
    public function testCollectWarnings() : void
    {
        ...
        $this->assertCount(2, $errors);
        $messages = ['Declaration of DeclarationWarning\\Bar::doFoo(int $i): void should be compatible with DeclarationWarning\\Foo::doFoo(): void', 'Parameter #1 $i of method DeclarationWarning\\Bar::doFoo() is not optional.'];
    ...
    private function runAnalyse(string $file) : array
    {
        ...
        $analyser = self::getContainer()->getByType(\TenantCloud\BetterReflection\Relocated\PHPStan\Analyser\Analyser::class);
        /** @var \PHPStan\File\FileHelper $fileHelper */
        $fileHelper = self::getContainer()->getByType(\TenantCloud\BetterReflection\Relocated\PHPStan\File\FileHelper::class);
