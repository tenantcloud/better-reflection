...
final class CachingExecutor implements \TenantCloud\BetterReflection\Relocated\React\Dns\Query\ExecutorInterface
{
    /**
    ...
    private $executor;
    private $cache;
    public function __construct(\TenantCloud\BetterReflection\Relocated\React\Dns\Query\ExecutorInterface $executor, \TenantCloud\BetterReflection\Relocated\React\Cache\CacheInterface $cache)
    {
    ...
    public function query(\TenantCloud\BetterReflection\Relocated\React\Dns\Query\Query $query)
    {
        ...
        return new \TenantCloud\BetterReflection\Relocated\React\Promise\Promise(function ($resolve, $reject) use($query, $id, $cache, $executor, &$pending, $that) {
            $pending->then(function ($message) use($query, $id, $cache, $executor, &$pending, $that) {
                ...
                return $pending = $executor->query($query)->then(function (\TenantCloud\BetterReflection\Relocated\React\Dns\Model\Message $message) use($cache, $id, $that) {
     ...
     * @internal
     */
    public function ttl(\TenantCloud\BetterReflection\Relocated\React\Dns\Model\Message $message)
    {
