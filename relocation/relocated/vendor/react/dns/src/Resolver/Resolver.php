...
use TenantCloud\BetterReflection\Relocated\React\Dns\RecordNotFoundException;
/**
 * @see ResolverInterface for the base interface
 */
final class Resolver implements \TenantCloud\BetterReflection\Relocated\React\Dns\Resolver\ResolverInterface
{
    private $executor;
    public function __construct(\TenantCloud\BetterReflection\Relocated\React\Dns\Query\ExecutorInterface $executor)
    {
        $this->executor = $executor;
    }
    public function resolve($domain)
    {
        return $this->resolveAll($domain, \TenantCloud\BetterReflection\Relocated\React\Dns\Model\Message::TYPE_A)->then(function (array $ips) {
            return $ips[\array_rand($ips)];
        });
    }
    public function resolveAll($domain, $type)
    {
        $query = new \TenantCloud\BetterReflection\Relocated\React\Dns\Query\Query($domain, $type, \TenantCloud\BetterReflection\Relocated\React\Dns\Model\Message::CLASS_IN);
        $that = $this;
        return $this->executor->query($query)->then(function (\TenantCloud\BetterReflection\Relocated\React\Dns\Model\Message $response) use($query, $that) {
     ...
     * @internal
     */
    public function extractValues(\TenantCloud\BetterReflection\Relocated\React\Dns\Query\Query $query, \TenantCloud\BetterReflection\Relocated\React\Dns\Model\Message $response)
    {
        // reject if response code indicates this is an error response message
        $code = $response->rcode;
        if ($code !== \TenantCloud\BetterReflection\Relocated\React\Dns\Model\Message::RCODE_OK) {
            switch ($code) {
                case \TenantCloud\BetterReflection\Relocated\React\Dns\Model\Message::RCODE_FORMAT_ERROR:
                    $message = 'Format Error';
                    break;
                case \TenantCloud\BetterReflection\Relocated\React\Dns\Model\Message::RCODE_SERVER_FAILURE:
                    $message = 'Server Failure';
                    break;
                case \TenantCloud\BetterReflection\Relocated\React\Dns\Model\Message::RCODE_NAME_ERROR:
                    $message = 'Non-Existent Domain / NXDOMAIN';
                    break;
                case \TenantCloud\BetterReflection\Relocated\React\Dns\Model\Message::RCODE_NOT_IMPLEMENTED:
                    $message = 'Not Implemented';
                    break;
                case \TenantCloud\BetterReflection\Relocated\React\Dns\Model\Message::RCODE_REFUSED:
                    $message = 'Refused';
                    break;
                default:
    ...
    private function valuesByNameAndType(array $answers, $name, $type)
    {
        ...
        $cnameRecords = $this->filterByType($named, \TenantCloud\BetterReflection\Relocated\React\Dns\Model\Message::TYPE_CNAME);
        if ($cnameRecords) {
