 ...
 * @coversNothing
 */
class ProjectCodeTest extends \TenantCloud\BetterReflection\Relocated\PHPUnit\Framework\TestCase
{
    ...
    public static function doSetUpBeforeClass()
    {
        ...
        $rc = new \ReflectionClass('TenantCloud\\BetterReflection\\Relocated\\Webmozart\\Assert\\Mixin');
    ...
    public function testHasNullOr($method)
    {
        ...
        if (!$correct) {
            $correct = \in_array($fullMethodName, self::$mixinMethodNames, \true);
        }
        if ($correct === \false) {
            $this->fail(\sprintf('All methods have a corresponding "nullOr" method, please add the "%s" method to the class level doc comment.', $fullMethodName));
        }
        $this->addToAssertionCount(1);
    }
    /**
    ...
    public function testHasAll($method)
    {
        ...
        if ($correct === \false) {
            $this->fail(\sprintf('All methods have a corresponding "all" method, please add the "%s" method to the class level doc comment.', $fullMethodName));
        }
        $this->addToAssertionCount(1);
    }
    /**
    ...
    public function testIsInReadme($method)
    {
        ...
        if ($correct === \false) {
            $this->fail(\sprintf('All methods must be documented in the README.md, please add the "%s" method.', $method));
        }
        $this->addToAssertionCount(1);
    }
    /**
    ...
    public function testHasThrowsAnnotation($method)
    {
        ...
        $this->assertNotFalse($doc, \sprintf('Expected a doc comment on the "%s" method, but none found', $method->getName()));
        $this->assertStringContainsString('@throws InvalidArgumentException', $doc, \sprintf('Expected method "%s" to have an @throws InvalidArgumentException annotation, but none found', $method->getName()));
    }
    /**
     * @dataProvider provideMethods
     *
    ...
    public function testHasCorrespondingStaticAnalysisFile($method)
    {
        ...
        $this->assertFileExists(__DIR__ . '/static-analysis/assert-' . $method->getName() . '.php');
    }
    public function testMixinIsUpToDateVersion()
    {
        ...
        $generator = new \TenantCloud\BetterReflection\Relocated\Webmozart\Assert\Bin\MixinGenerator();
    ...
    private function getMethods()
    {
        ...
        $rc = new \ReflectionClass('TenantCloud\\BetterReflection\\Relocated\\Webmozart\\Assert\\Assert');
        $methods = array();
